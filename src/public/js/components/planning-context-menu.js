...
export class PlanningContextMenu {
  /**
   ...
   */
  constructor(container, planningInstance) {
    this.page = planningInstance.page;
    this.container = container;
    this.planning = planningInstance;
    this.menuId = "planning-context-menu-popup";
    this.buttonId = "planning-context-menu-btn";
    this.menuContainer = document.createElement("div");
    this.isOpen = false;
    this.timeDropdowns = {};
    this.selectLogosPopup = new SelectLogosPopup(this.page, this.planning);
    this.renderButton();
  }
  ...
  openMenu() {
    ...
    if (!this.page.querySelector(`#${this.menuId}`)) {
      const menu = document.createElement("div");
      menu.id = this.menuId;
      menu.className =
        "w-fit absolute z-20 bg-white border border-gray-200 rounded-xl shadow-lg p-4 right-[2rem] animate-fade-in";
      ...
      // Ajout listeners pour les boutons radio d'affichage
      const defaultRangeRadio = menu.querySelector('input[value="default"]');
      const customRangeRadio = menu.querySelector('input[value="custom"]');

      defaultRangeRadio.addEventListener("change", () => {
        if (defaultRangeRadio.checked) {
          this.handleDisplayRangeChange("default");
        }
      });

      customRangeRadio.addEventListener("change", () => {
        if (customRangeRadio.checked) {
          this.handleDisplayRangeChange("custom");
        }
      });
      ...
      const menu = this.page.querySelector(`#${this.menuId}`);
      menu.classList.remove("hidden");
      this.isOpen = true;
    }

    // Positionner le menu (qu'il soit nouveau ou existant)
    const menu = this.page.querySelector(`#${this.menuId}`);
    const buttonRect = this.menuContainer.querySelector(`#${this.buttonId}`).getBoundingClientRect();
    const menuHeight = menu.offsetHeight;
    const windowHeight = window.innerHeight;

    // Calculer la position top en fonction de la formule inverse
    let topPosition = windowHeight - buttonRect.bottom - menuHeight;
    // S'assurer que le top ne dépasse pas 0
    topPosition = Math.min(0, topPosition);

    menu.style.top = `${topPosition}px`;
    menu.style.marginTop = "8px"; // Équivalent à mt-2
  ...
  getMenuHTML() {
    return `
      <div class="mb-2">
        <span class="font-bold text-gray-900">Affichage</span><br>
        <label class="flex items-center gap-2 mt-1">
          <input type="radio" name="display-range" value="default" class="accent-blue-600" checked />
          Plage par défaut
        </label>
        <label class="flex items-center gap-2 mt-1">
          <input type="radio" name="display-range" value="custom" class="accent-blue-600" />
          Plage ajustée
        </label>
      </div>
      <hr class="my-2">
  ...
  initDropdowns() {
    ...
    this.timeDropdowns = {
      "heure-debut": new TimeDropdown(
        ...
        6,
        17,
        this.planning.slotDuration,
        "planning-context-menu",
        _,
        this.planning.timeSlots[0]
      ),
      "heure-fin": new TimeDropdown(
        ...
        "planning-context-menu",
        _,
        this.planning.timeSlots[this.planning.timeSlots.length - 1]
      ),
      duree: new TimeSlot("dropdown-duree", "planning-context-menu"),
    };

    setTimeout(() => {
      for (const dropdown of Object.values(this.timeDropdowns)) {
        dropdown.init(this.page, this.planning, this);
  ...
  handleSave(replace = false) {
    ...
    input.type = "text";
    input.value = this.planning.name || "";
    input.className =
    ...
    saveBtn.onclick = async () => {
      ...
      this.planning.name = input.value;

      await this.planning.save(input.value); // Appelle la vraie sauvegarde ici
      overlay.remove();
      this.removePopup();
    };
    ...
    this.planning.page.appendChild(overlay);
  ...
  handlePrint() {
    this.selectLogosPopup.show("print");
  }
  handlePDF() {
    this.selectLogosPopup.show("pdf");
  }

  handleDisplayRangeChange(range) {
    if (range === "default") {
      this.planning.adjustTimeSlots = false;
      this.planning.generateTimeSlots();
    } else if (range === "custom") {
      if (this.planning.rows.length > 0) {
        this.planning.adjustTimeSlots = true;
        this.planning.generateTimeSlots();
      }
    }
  }
