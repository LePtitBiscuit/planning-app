...
import { AddRowModelMenu } from "./addrow-model-menu.js";
export class ConfigModelMenu {
  constructor() {}
  ...
  createTitleSection() {
    ...
    setTimeout(() => {
      ...
      input.addEventListener("input", (event) => {
        if (this.modelTable) {
          this.modelTable.updateTitle(event.target.value);
        } else {
          console.error("Model table instance not found in window object");
        }
      });
    }, 0); // 100ms de délai
  }
  ...
  createModelSection() {
    setTimeout(() => {
      ...
      window.modelDropdown = new FileDropdown("dropdown-model", "dropdown-model", "config", "model");
      window.modelDropdown.init(this.page, this.modelTable);
    }, 0);
  }
  ...
  createColumnsSection() {
    ...
    setTimeout(() => {
      ...
      this.columnInputs = new ColumnInputs("config", [], "model");
  ...
  createTimeRangeSection() {
    ...
    setTimeout(() => {
      ...
      this.timeDropdowns = {
        "heure-debut": new TimeDropdown("dropdown-heure-debut", "heure-debut", 6, 18, 15, "planning", _, _, "model"),
        "heure-fin": new TimeDropdown("dropdown-heure-fin", "heure-fin", 7, 18, 15, "planning", _, _, "model"),
        duree: new TimeSlot("dropdown-duree", "planning", "model"),
      };
      setTimeout(() => {
        for (const dropdown of Object.values(this.timeDropdowns)) {
          dropdown.init(this.page, this.modelTable, this);
  ...
  createCreateButton() {
    ...
    button.textContent = "Créer le modèle";

    container.appendChild(button);

    button.addEventListener("click", () => {
      // window.planningTable.addRow(this.getColumnValues(), this.getConfiguration());
      this.modelTable.firstSection = new AddRowModelMenu();
      this.modelTable.firstSection.initialize(this.page, this.modelTable);
      // Code pour affichier le bouton de paramètre du planning
      this.modelTable.showPlanningSettingsButton();
    });
  }
  ...
  loadModel(content) {
    ...
    this.modelTable.initialize(this.page, {
      title:
        content?.title.replace(/_/g, " ").replace(/-/g, "/") || "Modèle du " + new Date().toLocaleDateString("fr-FR"),
  ...
  updatePlanningTableTimeSlots() {
    ...
    this.modelTable.updateTimeSlots(startTime, endTime, duration);
  }
}
