...
import { ModelContextMenu } from "./model-context-menu.js";
const html2canvas = require("html2canvas");
export class ModelTableView {
  constructor() {
    this.title = `Modele du ${new Date().toLocaleDateString("fr-FR")}`;
  ...
  initialize(container, config) {
    ...
    titleContainer.className = "pb-4 text-center";
    ...
    this.tableContainer.className =
      "bg-white rounded-2xl border border-gray-200 shadow-md w-full mx-auto max-h-[580px] overflow-y-auto relative";
  ...
  showPlanningSettingsButton() {
    ...
    this.planningSettingsMenu = new ModelContextMenu(this.container, this);
  }
  ...
  generateTimeSlots(startTime, endTime, duration, changeRange = false, fromDurationDropdown = false) {
    ...
    if (changeRange && !this.adjustTimeSlots && !response.timeSlotError) {
      ...
      // Ne modifier que la durée des slots et pas les heures de début et de fin quand la vue est ajustée
      ...
      this.updateTimeSlotsColumns(newTimeSlots);
      return response;
      // En cas de changement de l'heure de début ou de fin quand la vue est ajustée
  ...
  updatePlanningTableTimeSlots(type, sender) {
    ...
    if (sender === "planning-context-menu") {
      ...
      // dropdownContainer = window.timeDropdowns3;
  ...
  createTableStructure() {
    ...
    for (let i = 0; i < 13; i++) {
      ...
      td.className = i === 0 ? "h-[40px]" : "timeslot-cell h-[40px]";
  ...
  addColumn(columnIndex = null) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      newCell.className = "h-[40px] border border-gray-200 text-center";
  ...
  deleteRow(row) {
    ...
    if (index > -1) {
      ...
      if (this.rows.length === 27) {
        if (this.firstSection.constructor.name === "AddRowModelMenu") {
          this.firstSection.toggleAddRowButton();
        }
      }

      if (this.firstSection && this.firstSection.constructor.name === "AddRowModelMenu") {
  ...
  addRow(columns, schedule, color, fromModel = false, index = null) {
    ...
    for (let i = 0; i <= this.columns.length - 1; i++) {
      const td = document.createElement("td");
      td.className = "h-[40px] border border-gray-200 text-center";
    ...
    // this.addRowDragListeners(dataRow, this.rows[this.rows.length - 1]);
            ...
            if (e.shiftKey) {
              ...
              if (cellIndex > 0) {
                ...
                if (currentRowIndex > 0) {
                  const prevRow = window.modelTable.rows[currentRowIndex - 1];
  ...
  async save(replace = false) {
    ...
    const planningData = {
      ...
      id: this.id,
      title: this.title.trim().replace(/\s+/g, "_").replace(/\//g, "-"),
    ...
    const status = await window.require("electron").ipcRenderer.invoke("save", "model", planningData);
